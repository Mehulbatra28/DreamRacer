...
namespace SAP2D.Editors {
    ...
    public class SAP_GraphDrawer {
        ...
        {
            float tDiameter = grid.TileDiameter;
            Gizmos.color = SAP2DPathfinder.singleton.EditorSettings.gridColor;
            Vector3 center3D = new Vector3(grid.Center.x, grid.Position.z, grid.Center.y);
            Vector3 size3D = new Vector3(grid.Size.x, 0, grid.Size.y);
            ...
            for (int x = 0; x < grid.Width; x++)
            {
                for (int y = 0; y < grid.Height; y++)
                {
                    ...
                    {
                        Vector3 pos3D = new Vector3(tile.WorldPosition.x, grid.Position.z, tile.WorldPosition.y);
                        Gizmos.color = SAP2DPathfinder.singleton.EditorSettings.obstacleColor;
                        Gizmos.DrawCube(pos3D, new Vector3(tDiameter, 0.1f, tDiameter));
                    }
                }
            }
        }
