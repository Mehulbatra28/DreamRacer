...
namespace SAP2D {
    ...
    public class SAP_GridSource
    {
        ...
        public bool UsePhysics3D = false;
        ...
        public void CalculateColliders(SAP_TileData startTile = null, SAP_TileData endTile = null)
        {
            ...
            for (int x = startTile.x; x <= endTile.x; x++)
            {
                for (int y = startTile.y; y <= endTile.y; y++)
                {
                    ...
                    if (UsePhysics3D == true)
                    {
                        // In 3D we map the 2D grid WorldPosition to a 3D position
                        // Assuming grid visual Y maps to 3D Z.
                        // We will use Grid's Position.z as the 3D Y (Height) for the physics check!
                        Vector3 pos3D = new Vector3(tile.WorldPosition.x, Position.z, tile.WorldPosition.y);
                        Collider[] colls3D = Physics.OverlapSphere(pos3D, tileDiameter / 4, ObstaclesLayer);
                        // INVERTED LOGIC: Only walk if a collider IS found on the selected layer!
                        tile.isWalkable = colls3D.Length > 0;
                    }
                    else if (UsePhysics2D == true)
                    {
